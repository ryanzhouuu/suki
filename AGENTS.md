...
- `npm test` — unit tests via Node's native `--test` runner over `tests/**/*.test.ts` (driven by `scripts/run-tests.mjs` using `tsx`; **not** Jest or Vitest)
- `npm run test:coverage` / `npm run test:coverage:check` — c8 coverage (measures `src/lib/**`; thresholds enforced)
...
Tests live in `tests/`, mirroring the `src/lib/` structure.
