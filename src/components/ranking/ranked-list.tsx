...
import { SeriesGroupDetailsDialog } from "@/components/library/series-group-details-dialog";
    ...
    <>
      <ol className="space-y-2">
        {visibleRankings.map((row) => {
          ...
          const libraryGroup = libraryGroupsBySeriesId?.[row.series_id] ?? null;
          return (
            <li
              key={row.id}
              className="group flex flex-wrap items-center gap-3 rounded-card border border-line bg-surface px-3 py-2.5 transition-colors hover:border-accent"
            >
              <span
              ...
              {libraryGroup || editable ? (
                <div className="ml-auto flex w-full shrink-0 flex-wrap justify-end gap-1.5 sm:w-auto">
                  {libraryGroup ? (
                    <button
                      type="button"
                      onClick={() => setSelectedGroup(libraryGroup)}
                      className="inline-flex min-h-8 items-center justify-center rounded-full border border-line-strong bg-surface px-2.5 text-xs font-medium text-ink transition-colors hover:border-accent hover:text-accent"
                    >
                      Details
                    </button>
                  ) : null}
