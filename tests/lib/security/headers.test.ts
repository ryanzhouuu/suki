...
const supabaseHostname = "abcdefgh.supabase.co";
...
describe("buildSecurityHeaders", () => {
  it("includes frame-ancestors 'none' and object-src 'none' in the CSP", () => {
    const headers = buildSecurityHeaders({ isDev: false, supabaseHostname });
  ...
  it("includes 'unsafe-eval' in script-src only when isDev is true", () => {
    const devHeaders = buildSecurityHeaders({ isDev: true, supabaseHostname });
    const devCsp = getHeader(devHeaders, "Content-Security-Policy")!;
    const devScriptSrc = devCsp.split(";").find((d) => d.trim().startsWith("script-src"));
    assert.match(devScriptSrc!, /'unsafe-eval'/);

    const prodHeaders = buildSecurityHeaders({ isDev: false, supabaseHostname });
    const prodCsp = getHeader(prodHeaders, "Content-Security-Policy")!;
    const prodScriptSrc = prodCsp.split(";").find((d) => d.trim().startsWith("script-src"));
    assert.doesNotMatch(prodScriptSrc!, /'unsafe-eval'/);
  });

  it("includes the passed Supabase hostname in connect-src", () => {
    const headers = buildSecurityHeaders({ isDev: false, supabaseHostname });
    const csp = getHeader(headers, "Content-Security-Policy")!;
    const connectSrc = csp.split(";").find((d) => d.trim().startsWith("connect-src"));
    assert.match(connectSrc!, new RegExp(`https://${supabaseHostname}`));
  });

  it("includes the Supabase hostname in img-src for avatar/banner images", () => {
    const headers = buildSecurityHeaders({ isDev: false, supabaseHostname });
    const csp = getHeader(headers, "Content-Security-Policy")!;
    const imgSrc = csp.split(";").find((d) => d.trim().startsWith("img-src"));
    assert.match(imgSrc!, new RegExp(`https://${supabaseHostname}`));
  });

  it("includes known image hosts (anilist, crunchyroll, youtube thumbnails) in img-src", () => {
    const headers = buildSecurityHeaders({ isDev: false, supabaseHostname });
    const csp = getHeader(headers, "Content-Security-Policy")!;
    const imgSrc = csp.split(";").find((d) => d.trim().startsWith("img-src"));
  ...
  it("does not include a frame-src directive (no iframes are used in this app)", () => {
    const headers = buildSecurityHeaders({ isDev: false, supabaseHostname });
    const csp = getHeader(headers, "Content-Security-Policy")!;
    assert.doesNotMatch(csp, /frame-src/);
  });

  it("allows Vercel Analytics and Speed Insights origins in script-src and connect-src", () => {
    const headers = buildSecurityHeaders({ isDev: false, supabaseHostname });
    const csp = getHeader(headers, "Content-Security-Policy")!;
